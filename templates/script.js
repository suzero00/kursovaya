...
async function loginUser(username, password) {
    ...
    if (res.ok) {
        saveUser(data);
        location.href = "index.html";  // переход на главную
    } else {
        alert(data.detail);
    }
}
...
async function loadMovies(){
    ...
    movies.forEach(movie => {
        ...
        card.innerHTML = `
            ...
            <h3>${movie.title}</h3>
        `;
        card.onclick = () => loadSessions(movie.id);
...
async function loadSessions(movieId){
    ...
    sessions.forEach(session => {
        ...
        `;
        card.onclick = () => showSeats(session);
        container.appendChild(card);
    });
...
function showSeats(session) {
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        seat.onclick = async () => {
            ...
            if (isMySeat) {
                if (!confirm(`Вы уверены, что хотите отменить бронь места №${i}?`)) return;
            } else {
                if (!confirm(`Вы уверены, что хотите забронировать место №${i}?`)) return;
            }
